...
export default async function ProfilePage() {
  ...
  return (
    <main className="mx-auto w-full px-4 py-5 sm:mx-auto sm:max-w-6xl sm:px-6 sm:py-6 lg:px-8 lg:py-10">
      <section className="rounded-[1.75rem] border border-white/70 bg-white/75 p-5 shadow-2xl shadow-blush-200/40 backdrop-blur sm:rounded-[2rem] sm:p-6 md:p-7">
        <p className="text-xs font-semibold text-blush-700 sm:text-sm">TA 档案</p>
        <h1 className="mt-1.5 font-display text-3xl font-semibold leading-none tracking-normal text-ink-900 sm:mt-2 sm:text-5xl">
        ...
        {!profile ? (
          <div className="mt-5 rounded-2xl bg-blush-50 p-4 text-sm leading-6 text-ink-700 sm:mt-7 sm:rounded-3xl sm:p-5">
            <p className="font-black text-ink-900">还没有 Crush 档案</p>
            <p>先完成建档流程，事实、推测和雷区才会在这里逐步沉淀。</p>
            <Link
              className="mt-2.5 inline-flex rounded-full bg-white px-4 py-2 font-bold text-ink-900 sm:mt-3"
          ...
          <div className="mt-5 grid gap-5 sm:mt-7 lg:grid-cols-[1fr_300px]">
            ...
            <div className="grid gap-4 sm:gap-5">
              ...
              {!confirmedFacts.length && !inferredTraits.length && !boundaries.length && (
                <div className="rounded-2xl bg-blush-50 p-4 text-sm leading-6 text-ink-700 sm:rounded-3xl sm:p-5">
            ...
            <aside className="space-y-3 sm:space-y-4">
              ...
              {!recentEvents.length && !activeSignals.length && !pendingInferences.length && (
                <div className="rounded-2xl border border-blush-100 bg-white/70 p-4 shadow-sm sm:rounded-3xl sm:p-5">
                  <p className="text-[10px] font-black uppercase tracking-widest text-blush-700 sm:text-xs">现实观察</p>
...
function InfoCard({
  ...
  return (
    <div className={`rounded-2xl ${toneClasses[tone]} p-3 sm:p-5`}>
      <p className="text-[10px] font-medium leading-4 text-ink-700/70 sm:text-xs sm:leading-5">{label}</p>
      <p className={`mt-1 text-base font-semibold leading-5 sm:mt-2 sm:text-2xl sm:leading-8 ${textClasses[tone]}`}>{value}</p>
    </div>
  );
}
...
function TraitCard({
  ...
  return (
    <article className="rounded-xl bg-white p-3 shadow-sm sm:rounded-2xl sm:p-4">
      <div className="flex items-start justify-between gap-2">
        <h3 className="font-semibold text-ink-900">{trait.label}</h3>
        <span className={`shrink-0 rounded-full border px-2 py-0.5 text-[10px] font-bold sm:px-2.5 sm:py-0.5 sm:text-xs ${config.color}`}>
          {config.badge}
        </span>
      </div>
      {trait.description && (
        <p className="mt-1.5 text-xs leading-relaxed text-ink-600 sm:mt-2 sm:text-sm">{trait.description}</p>
      )}
      {trait.confidence != null && (
        <p className="mt-1 text-[10px] text-ink-400 sm:mt-2 sm:text-xs">
...
function ProfileSection({
  ...
  return (
    <div className={`rounded-2xl border ${borderColors[tone]} bg-white/50 p-4 sm:rounded-3xl sm:p-5`}>
      <div className="mb-3 sm:mb-4">
        <p className={`text-[10px] font-black uppercase tracking-widest ${titleColors[tone]} sm:text-xs`}>{title}</p>
        {subtitle && <p className="mt-1 hidden text-[10px] text-ink-400 sm:block sm:text-xs">{subtitle}</p>}
      </div>
...
function RealityLayerPanel({
  ...
  return (
    <div className="rounded-2xl border border-blush-100 bg-white/70 p-4 shadow-sm sm:rounded-3xl sm:p-5">
      <p className="text-[10px] font-black uppercase tracking-widest text-blush-700 sm:text-xs">{title}</p>
      <p className="mt-1 text-xs leading-4 text-ink-500 sm:mt-2 sm:text-sm sm:leading-5">{description}</p>
      ...
      {items.length > 0 ? (
        ...
        <div className="mt-2 rounded-xl bg-blush-50 px-2.5 py-2 text-xs leading-4 text-ink-500 sm:mt-3 sm:rounded-2xl sm:px-3 sm:py-2.5 sm:text-xs sm:leading-5">
          暂无
        </div>
      )}
    </div>
...
function RealityItemCard({ item, type }: { item: RealityItem; type: RealityLayerType }) {
  if (type === "events") {
    return (
      <div className="rounded-xl border border-blush-100 bg-blush-50/60 px-2.5 py-2 text-xs leading-4 text-ink-700 sm:px-3 sm:py-2.5 sm:text-xs sm:leading-5">
        {item.occurredAtText && (
          <span className="mr-1 font-bold text-blush-600">[{item.occurredAtText}]</span>
        )}
        {item.eventText}
      </div>
    );
  ...
  if (type === "signals") {
    ...
    return (
      <div className={`rounded-xl border px-2.5 py-2 text-xs leading-4 ${polarityConfig.color} sm:px-3 sm:py-2 sm:text-xs sm:leading-5`}>
        <div className="flex items-center justify-between gap-2">
          <span className="font-semibold">{item.label}</span>
          <span className="shrink-0 rounded-full border bg-white px-1 py-0.5 text-[10px] font-bold sm:px-1.5 sm:py-0.5 sm:text-[10px]">
            {polarityConfig.badge}
          </span>
        </div>
        {item.description && <p className="mt-0.5 text-ink-600">{item.description}</p>}
        {item.confidence != null && (
          <p className="mt-0.5 text-ink-400">{Math.round(item.confidence * 100)}%</p>
        )}
      </div>
    );
  }
  ...
  // inferences
  return (
    <div className="rounded-xl border border-violet-100 bg-violet-50/60 px-2.5 py-2 text-xs leading-4 text-violet-700 sm:px-3 sm:py-2 sm:text-xs sm:leading-5">
      <span className="font-semibold">{item.label}</span>
      {item.description && <p className="mt-0.5 text-violet-600">{item.description}</p>}
      {item.confidence != null && (
        <p className="mt-0.5 text-violet-400">置信度 {Math.round(item.confidence * 100)}%</p>
      )}
    </div>
  );
}